...
export default function Home() {
  ...
  const processedManga = useMemo(() => {
    ...
    if (activeTab !== "ทั้งหมด") {
      if (activeTab === "✅ จบแล้ว") {
        result = result.filter((m: any) => m.status === "completed" || m.status === "จบแล้ว" || m.status === "✅ จบสมบูรณ์ (Completed)");
      } else {
        ...
        <section className="relative w-full aspect-[2.5/1] sm:aspect-[3/1] md:aspect-[3.5/1] bg-black border-b border-white/5 overflow-hidden">
          ...
          <div className="absolute bottom-2 md:bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 md:gap-2.5 z-20">
            {featuredManga.map((_, i) => (
              <button key={i} onClick={() => setCurrentBanner(i)} className={`h-1 md:h-1.5 rounded-full transition-all duration-300 ${i === currentBanner ? 'w-6 md:w-10 bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]' : 'w-1.5 md:w-2 bg-white/30 hover:bg-white/60'}`} />
            ))}
          </div>
        </section>
      )}
      ...
      <div className="mt-10 mb-8 px-4 md:px-6 flex flex-col items-center">
        <div className="flex items-center gap-6 mb-5 scale-110 md:scale-125 transition-transform duration-500">
          <div className="relative">
              ...
              <img src="/profile.png" className="relative w-16 h-16 md:w-20 md:h-20 rounded-full border-2 border-indigo-500 p-0.5 shadow-2xl bg-[#0D0D0D]" alt="Profile" />
          </div>
        ...
        <div className="w-full max-w-4xl space-y-5">
          <div className="flex items-stretch gap-2 md:gap-3 w-full">
            <div className="relative group flex-1">
              <input 
                ...
                className="w-full h-full bg-[#111] border border-white/10 rounded-2xl py-4 md:py-4.5 px-5 pr-12 text-[16px] focus:border-indigo-500 outline-none transition-all shadow-2xl" 
              />
            ...
            <Link href="/bookmarks" className="flex items-center justify-center bg-[#111] border border-white/10 hover:border-indigo-500 hover:text-indigo-500 text-gray-400 rounded-2xl w-[56px] md:w-[64px] transition-all shadow-2xl active:scale-95 group relative flex-shrink-0" title="ชั้นหนังสือของฉัน">
              <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="md:w-[26px] md:h-[26px] group-hover:fill-indigo-500/20 transition-all"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
              <span className="absolute top-3 right-3 md:top-3.5 md:right-3.5 w-2 h-2 md:w-2.5 md:h-2.5 bg-indigo-500 rounded-full shadow-[0_0_8px_rgba(99,102,241,1)] border border-[#111]"></span>
            </Link>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="flex gap-1.5 bg-[#111] p-1.5 rounded-2xl border border-white/5 w-full overflow-x-auto no-scrollbar">
              {['ทั้งหมด', '🇰🇷 มันฮวา', '🔞 ติดเรท', '✅ จบแล้ว'].map((tab) => (
                <button 
                  ...
                  className={`flex-1 px-2 sm:px-4 py-2.5 rounded-xl text-[10px] sm:text-[11px] font-black uppercase whitespace-nowrap transition-all ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}
                >
                  {tab}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto justify-center flex-wrap sm:flex-nowrap">
              ...
              {/* ✨ ปุ่มวาร์ปไปเว็บสาววาย (ขนาดเท่าเว็บสาววายเป๊ะ + โฮเวอร์สีชมพู) */}
              {/* <a 
                href="https://bltranslate.vercel.app" 
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-3 py-3 md:px-4 md:py-3.5 bg-[#111] hover:bg-pink-500/10 text-gray-400 hover:text-pink-400 border border-white/10 hover:border-pink-500/30 rounded-2xl text-[10px] md:text-[11px] font-black transition-all shadow-lg active:scale-95 whitespace-nowrap flex-shrink-0"
                title="ไปเว็บสาววายขอแปล"
              >
                <ExternalLink size={14} className="text-pink-500 flex-shrink-0" />
                <span className="hidden sm:inline">อ่านแนววายคลิกเลย</span>
              ...
              {/* ✨ ปุ่มตารางอัปเดต (ธีม Indigo) */}
              <button 
                onClick={() => setShowSchedule(!showSchedule)} 
                className={`p-3 md:p-3.5 border rounded-2xl shadow-lg transition-all active:scale-90 flex-shrink-0 ${showSchedule ? 'bg-indigo-600 border-indigo-500 text-white shadow-indigo-500/20' : 'bg-[#111] border-white/10 text-gray-400 hover:text-indigo-500'}`}
                title="ตารางอัปเดต"
              >
                <Calendar size={18} className="md:w-5 md:h-5" />
              </button>
              
              {/* ✨ ปุ่ม Grid (ไซส์ไอคอน 16px และขยายเป็น 18px ในจอใหญ่ + ธีม Indigo) */}
              <div className="flex items-center gap-1.5 bg-[#111] p-1.5 rounded-2xl border border-white/10 shadow-lg flex-shrink-0">
                  <button onClick={() => setGridCols(1)} className={`p-2 md:p-2.5 rounded-xl transition-all ${gridCols === 1 ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-gray-500 hover:bg-white/5'}`}><Square size={16} className="md:w-[18px] md:h-[18px]" /></button>
                  <button onClick={() => setGridCols(2)} className={`p-2 md:p-2.5 rounded-xl transition-all ${gridCols === 2 ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-gray-500 hover:bg-white/5'}`}><Grid2X2 size={16} className="md:w-[18px] md:h-[18px]" /></button>
                  <button onClick={() => setGridCols(3)} className={`p-2 md:p-2.5 rounded-xl transition-all ${gridCols === 3 ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-gray-500 hover:bg-white/5'}`}><Grid3X3 size={16} className="md:w-[18px] md:h-[18px]" /></button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-7xl mx-auto px-2 md:px-8 mt-6">
        {!isSearching ? (
          <div className="space-y-12 md:space-y-16">
                      ...
                      {days.map((day) => (
                        <button
                          ...
                          className={`py-3 md:py-4 rounded-xl text-[9px] md:text-[11px] font-black transition-all ${
                            ...
                            ? "bg-indigo-600 text-white shadow-[0_10px_20px_-5px_rgba(99,102,241,0.5)] scale-105" 
            ...
            <section ref={catalogRef} className="pt-10 border-t border-white/5">
              <div className="flex items-center gap-3 mb-10 px-2">
                <div className="p-3 bg-indigo-500/10 rounded-xl border border-indigo-500/20 shadow-lg">
              ...
              {!isLoading && displayLimit < processedManga.length && (
                <div className="mt-20 flex justify-center">
                  <button 
                    onClick={() => setDisplayLimit(prev => prev + 12)} 
                    className="px-14 py-4.5 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[11px] rounded-2xl uppercase tracking-[0.3em] transition-all shadow-xl active:scale-95 border border-white/10"
                  >
                    แสดงเพิ่มเติม
                  </button>
                </div>
          ...
          <div className={`grid ${getGridClass()} gap-3 md:gap-5`}>
            {processedManga.map((m: any) => <MangaCard key={m.slug} manga={m} onClick={() => setSelectedManga(m)} gridMode={gridCols} />)}
          </div>
        )}
      </div>
