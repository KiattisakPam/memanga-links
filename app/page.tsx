...
export default function Home() {
  ...
  const [gridCols, setGridCols] = useState(3); // 1: Square, 2: Grid2x2, 3: Grid3x3
  ...
  // ✨ คำนวณ Grid Class ให้ล้อตาม Resizer แบบ Yaksha
  const getGridClass = () => {
    if (gridCols === 1) return "grid-cols-1 sm:grid-cols-2"; // ปุ่มใหญ่
    if (gridCols === 2) return "grid-cols-2 sm:grid-cols-3 md:grid-cols-4"; // ปุ่มกลาง
    return "grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6"; // ปุ่มเล็ก (ปกติ)
  };
    ...
    <div className="min-h-screen w-full bg-[#050505] text-white pb-20 overflow-x-hidden font-sans selection:bg-indigo-500/30">
      ...
      <div className="mt-8 mb-10 w-full max-w-5xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-white/5 pb-8">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-indigo-400 text-[10px] font-bold tracking-[0.3em] mt-1 uppercase">งานแปลคุณภาพระดับพรีเมียม</p>
          </div>
        </div>

        <div className="flex items-center gap-5">
          <a href="#" className="text-gray-400 hover:text-blue-500 transition-colors"><FacebookIcon /></a>
          <a href="#" className="text-gray-400 hover:text-red-600 transition-colors"><YoutubeIcon /></a>
          <a href="#" className="text-gray-400 hover:text-pink-500 transition-colors"><TikTokIcon /></a>
        </div>
      </div>
      ...
      <div className="sticky top-4 z-[50] w-full max-w-4xl mx-auto px-4 mb-14 space-y-5">
        <div className="relative group">
          <input type="text" placeholder="ค้นหามังฮวาที่คุณรัก..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="relative w-full bg-[#0D0D0D]/95 backdrop-blur-3xl border border-white/10 rounded-2xl py-4 px-10 text-base focus:outline-none focus:border-indigo-500/50 transition-all shadow-2xl" />
          <Search className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-500" size={22} />
        </div>

        {!isSearching && (
          <div className="flex items-center justify-between gap-4 overflow-x-auto no-scrollbar">
            <div className="flex gap-1.5 p-1 bg-[#0D0D0D] rounded-xl border border-white/5">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setSelectedManga(allManga[Math.floor(Math.random() * allManga.length)])} className="p-2.5 bg-white text-black rounded-xl shadow-xl active:scale-90"><Shuffle size={18} /></button>
              {/* ✨ Resizer แบบ Yaksha (1, 2, 3) */}
              <div className="flex items-center gap-1.5 bg-[#0D0D0D] p-1.5 rounded-xl border border-white/5 shadow-xl">
                  <button onClick={() => setGridCols(1)} className={`p-2 rounded-lg transition-all ${gridCols === 1 ? 'bg-red-600 text-white' : 'text-gray-600 hover:text-gray-200'}`}><Square size={16}/></button>
                  <button onClick={() => setGridCols(2)} className={`p-2 rounded-lg transition-all ${gridCols === 2 ? 'bg-red-600 text-white' : 'text-gray-600 hover:text-gray-200'}`}><Grid2X2 size={16}/></button>
                  <button onClick={() => setGridCols(3)} className={`p-2 rounded-lg transition-all ${gridCols === 3 ? 'bg-red-600 text-white' : 'text-gray-600 hover:text-gray-200'}`}><Grid3X3 size={16}/></button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="w-full max-w-7xl mx-auto px-2 md:px-8">
        {isSearching ? (
          ...
          <div className="space-y-12 md:space-y-16">
            ...
            <section ref={catalogRef} className="pt-16 border-t border-white/5">
              ...
              {displayLimit < processedManga.length && (
                <div className="mt-20 flex justify-center">
                  <button onClick={() => setDisplayLimit(prev => prev + 12)} className="px-12 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[11px] rounded-2xl uppercase tracking-[0.3em] transition-all shadow-xl active:scale-95">แสดงเพิ่มเติม</button>
                </div>
              )}
            </section>
          </div>
