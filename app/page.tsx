...
export default function Home() {
  ...
  const [gridCols, setGridCols] = useState(2); // เริ่มต้นที่ระดับกลาง
      ...
      <div className="mt-6 mb-6 px-4 md:px-6 flex flex-col items-center">
        ...
        <div className="w-full max-w-4xl space-y-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            {/* Tools (Shuffle & Resizer) */}
            <div className="flex items-center gap-3 w-full md:w-auto justify-center">
              <button 
                onClick={() => setSelectedManga(allManga[Math.floor(Math.random() * allManga.length)])} 
                className="p-3 bg-[#111] border border-white/10 rounded-2xl text-gray-400 hover:text-indigo-500 shadow-lg active:scale-90 transition-all"
                title="สุ่มเรื่อง"
              >
                <Shuffle size={20} />
              </button>
        ...
        {!isSearching ? (
          <div className="space-y-8 md:space-y-12">
            ...
            <section ref={catalogRef} className="pt-8 border-t border-white/5">
              ...
              <div className={`grid ${getGridClass()} gap-2 md:gap-4 transition-all duration-500`}>
              ...
              {displayLimit < processedManga.length && (
                <div className="mt-16 flex justify-center">
                  <button 
                    ...
                    className="px-12 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[11px] rounded-2xl uppercase tracking-[0.3em] transition-all shadow-xl active:scale-95 border border-white/10"
                  >
                    แสดงเพิ่มเติม
                  </button>
                </div>
