...
// ✨ ปรับ Query ให้ดึง updateDay (ตารางอัปเดต) มาด้วย ✨
      ...
      <div className="mt-10 mb-8 px-4 md:px-6 flex flex-col items-center">
        ...
        <div className="w-full max-w-4xl space-y-5">
          <div className="relative group">
            <input 
              type="text" 
              placeholder="ค้นหามังฮวาที่คุณรัก..." 
              value={searchQuery} 
              onChange={(e) => setSearchQuery(e.target.value)} 
              className="w-full bg-[#111] border border-white/10 rounded-2xl py-4.5 px-10 text-base focus:border-indigo-500 outline-none transition-all shadow-2xl" 
            />
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="flex gap-1.5 bg-[#111] p-1.5 rounded-2xl border border-white/5 w-full md:w-auto overflow-x-auto no-scrollbar">
              {['ทั้งหมด', '🇰🇷 มันฮวา', '🔞 ติดเรท'].map((tab) => (
                <button 
                  ...
                  className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase whitespace-nowrap transition-all ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                >
                  {tab}
                </button>
              ))}
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-center">
              <button 
                ...
                className="p-3.5 bg-[#111] border border-white/10 rounded-2xl text-gray-400 hover:text-indigo-500 shadow-lg active:scale-90 transition-all"
                title="สุ่มเรื่อง"
              >
                <Shuffle size={20} />
              </button>
              ...
              <button 
                ...
                className={`p-3.5 border rounded-2xl shadow-lg transition-all active:scale-90 ${showSchedule ? 'bg-indigo-600 border-indigo-500 text-white shadow-indigo-500/20' : 'bg-[#111] border-white/10 text-gray-400 hover:text-indigo-500'}`}
                title="ตารางอัปเดต"
              >
                <Calendar size={20} />
              </button>
              ...
              <div className="flex items-center gap-1.5 bg-[#111] p-1.5 rounded-2xl border border-white/10 shadow-lg">
                  <button onClick={() => setGridCols(1)} className={`p-2.5 rounded-xl transition-all ${gridCols === 1 ? 'bg-red-600 text-white shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}><Square size={18}/></button>
                  <button onClick={() => setGridCols(2)} className={`p-2.5 rounded-xl transition-all ${gridCols === 2 ? 'bg-red-600 text-white shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}><Grid2X2 size={18}/></button>
                  <button onClick={() => setGridCols(3)} className={`p-2.5 rounded-xl transition-all ${gridCols === 3 ? 'bg-red-600 text-white shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}><Grid3X3 size={18}/></button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-7xl mx-auto px-2 md:px-8">
        {!isSearching ? (
          <div className="space-y-12 md:space-y-16">
            ...
            {/* ✨ ตารางอัปเดตรายสัปดาห์ (แก้ไข Grid และภาษาไทย) ✨ */}
            <AnimatePresence>
              {showSchedule && (
                <motion.div 
                ...
                >
                  <div className="w-full bg-[#0D0D0D] border border-white/5 rounded-[2.5rem] md:rounded-[3.5rem] p-4 md:p-10 shadow-2xl relative mb-12">
                    ...
                    {/* ✨ แก้ไขเป็น Grid 7 คอลัมน์ ไม่ต้องเลื่อนแล้ว ✨ */}
